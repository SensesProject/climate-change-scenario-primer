const data = [{
  ...
  name: 'Agriculture',
  category: 'Economic Growth and Development',
  models: ['AIM/CGE', 'GCAM4', 'REMIND-MAGPIE'],
  x: 72.04,
  y: 49.03
}, {
  id: 'c0p6',
  ...
  name: 'Use of Land',
  category: 'Land use',
  models: ['AIM/CGE', 'GCAM4', 'IMAGE', 'REMIND-MAGPIE'],
  x: 56.71,
  y: 67.90 + 6.05
}, {
  id: 'c1p2',
...
//   y: 83.85
  ...
  category: 'Electricity and heat',
  models: ['GCAM4', 'IMAGE', 'REMIND-MAGPIE'],
  x: 23.88,
  y: 60.72
}, {
  id: 'c2p21',
...
data.filter(d => d.id.match(/c6p[0-5]/) || d.id.match(/c2p([1-9]$|1[0-5]$)/)).forEach((d, i) => {
  d.selectedId = i > 14 ? i + 3 : i
})

data.filter(d => d.id.match(/c2p([1-9]$|1[0-5]$)/)).forEach((d) => {
  d.color = 'yellow'
})

data.filter(d => d.id.match(/c6p[0-5]/)).forEach((d) => {
  d.color = 'red'
})
